	...
	for (let i = 1; i < arr.length; i++) {
		if (arr[i] > arr[i - 1]) {
			...
			if (peakCandidate !== null) {
				result.pos.push(peakCandidate);
				result.peaks.push(arr[peakCandidate]);
				peakCandidate = null;
			}
		}
	}
...
deepEqual(pickPeaks([1, 2, 3, 6, 4, 1, 2, 3, 2, 1]), {
	pos: [3, 7],
	peaks: [6, 3],
});
deepEqual(pickPeaks([3, 2, 3, 6, 4, 1, 2, 3, 2, 1, 2, 3]), {
	pos: [3, 7],
	peaks: [6, 3],
});
deepEqual(pickPeaks([3, 2, 3, 6, 4, 1, 2, 3, 2, 1, 2, 2, 2, 1]), {
	pos: [3, 7, 10],
	peaks: [6, 3, 2],
});
deepEqual(pickPeaks([2, 1, 3, 1, 2, 2, 2, 2, 1]), {
	pos: [2, 4],
	peaks: [3, 2],
});
deepEqual(pickPeaks([2, 1, 3, 1, 2, 2, 2, 2]), { pos: [2], peaks: [3] });
deepEqual(pickPeaks([2, 1, 3, 2, 2, 2, 2, 5, 6]), {
	pos: [2],
	peaks: [3],
});
