...
function howMuchCoffee(events) {
	const valid = new Set(['cw', 'dog', 'cat', 'movie']);

	const total = events.reduce((sum, e) => {
		const key = e.toLowerCase();
		if (!valid.has(key)) return sum;
		return sum + (e === key ? 1 : 2);
	}, 0);
...
strictEqual(howMuchCoffee([]), 0);
strictEqual(howMuchCoffee(['cw']), 1);
strictEqual(howMuchCoffee(['CW']), 2);
strictEqual(howMuchCoffee(['cw', 'CAT']), 3);
strictEqual(howMuchCoffee(['cw', 'CAT', 'DOG']), 'You need extra sleep');
strictEqual(howMuchCoffee(['cw', 'CAT', 'cw=others']), 3);
