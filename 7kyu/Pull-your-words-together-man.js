...
function sentencify(words) {
    words.forEach(string => string.toLowerCase());
    let asSentence = words.join(' ');
    if(asSentence[-1] !== '.') asSentence += '.';
    asSentence = String(asSentence).charAt(0).toUpperCase() + String(asSentence).slice(1);
    return asSentence;
}