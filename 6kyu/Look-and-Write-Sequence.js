...
function lookAndWrite(n) {
	...
	while (n > 0) {
		...
		sequence = '';
		for (const elem of Object.keys(freqMap)) {
			sequence += `${freqMap[elem]}${elem}`;
		}
		n--;
		result += sequence;
	}
...
assertEqual(lookAndWrite(0), 2);
assertEqual(lookAndWrite(1), 4);
assertEqual(lookAndWrite(2), 8);
assertEqual(lookAndWrite(3), 14);
assertEqual(lookAndWrite(4), 22);
