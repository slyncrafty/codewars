...
const sendMessage = (message) => {
	...
	const getKeyByChar = (ch) => {
		for (const [key, char] of Object.entries(keyLookUp)) {
			const index = char.indexOf(ch);
			if (index !== -1) return key.repeat(index + 1);
		}
		return null;
	};
	...
	for (let i = 0; i < message.length; i++) {
		...
		if (/\d/.test(ch) || ['*', '#'].includes(ch)) {
			const last = sequence.at(-1) || '';
			if (last && last.at(-1) === ch) {
				sequence.push(' ');
			}
			sequence.push(ch + '-');
			continue;
		...
		if (isLetter) {
			const toUpper = ch === ch.toUpperCase();
			if (toUpper !== upperToggle) {
				sequence.push('#');
		...
		const seq = getKeyByChar(ch);
		if (!seq) continue;

		const last = sequence.at(-1) || '';

		if (last && last.at(-1) === seq.at(0)) {
			sequence.push(' ');
		}
		sequence.push(seq);
	}
...
strictEqual(sendMessage('Def Con 1!'), '#3#33 3330#222#666 6601-1111');

strictEqual(sendMessage('Hi'), '#44#444');

strictEqual(sendMessage('Aa'), '#2#2');

strictEqual(sendMessage('a7BK'), '27-#2255');
