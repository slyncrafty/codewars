...
// Solution
function roomMates(rooms, floor) {
	return rooms.filter((e, i) => e && Math.floor(i / 6) === floor - 1);
}
...
const assertSimilar = (actual, expected) => {
	const eqaul = (a, b) => {
		if (a === b) return true;
			...
			return a.every((e, i) => eqaul(e, b[i]));
		}
	};
	if (eqaul(actual, expected)) console.log('✅Test Passed');
	else console.log('❌Test Failed');
};

assertSimilar(roomMates([], 1), []);
let a = ['foo', 'bar.'];
assertSimilar(roomMates(['foo', 'bar.'], 1), a);
a = ['foo', 'bar', 'foobar', 'barfoo', 'foofoo', 'barbar'];
assertSimilar(roomMates(a, 1), a);
assertSimilar(roomMates(a, 2), []);
a = ['foo', 'bar', 'foobar', 'barfoo', 'foofoo', 'barbar'];
let b = ['ying', 'yang', 'yingyang', 'yangying', 'yangyang', 'yingying'];
assertSimilar(roomMates(a.concat(b), 1), a);
assertSimilar(roomMates(a.concat(b), 2), b);
