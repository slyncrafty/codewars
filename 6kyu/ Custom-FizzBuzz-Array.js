...
let fizzBuzzCustom = function(stringOne = 'Fizz', stringTwo = 'Buzz', numOne = 3, numTwo = 5) {
  ...
  for(let i = 1; i <= 100; i++) {
    let str = "";
    if(i % numOne === 0) str += stringOne;
    if(i % numTwo === 0) str += stringTwo;
    res.push(str || i);
  }
  return res;
};
...
assertEq(fizzBuzzCustom()[15], 16);
assertEq(fizzBuzzCustom()[44], "FizzBuzz");
assertEq(fizzBuzzCustom('Hey', 'There')[25], 26);
assertEq(fizzBuzzCustom('Hey', 'There')[11], "Hey");
assertEq(fizzBuzzCustom("What's ", "up?", 3, 7)[80], "What's ");
